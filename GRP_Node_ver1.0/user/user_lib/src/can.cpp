...
uint16_t CAN::filterAddress[4*14];
...
void CAN::filterAdd(uint16_t id1){
	//filterCntはCANクラスのfilterAddress[]用
	//filterAddress[]はCANの設定的には特に意味がない
	if(id1 != 0){
		filterAddress[filterCnt] = id1;
		filterCnt++;
	}

	CANFilterAdd(filterAddress[(filterCnt/4)],filterAddress[(filterCnt/4)+1],filterAddress[(filterCnt/4)+2],filterAddress[(filterCnt/4)+3]);
}
...
/*
    ...
    CAN_ITConfig(CAN1,CAN_IT_FMP0,ENABLE);	//FIFO0受信割込み
}
...
void CAN1Send(uint16_t id,uint8_t length,uint8_t data[8]){
    ...
    while(CANTXOK != CAN_TransmitStatus(CAN1,0)){
    	if(millis() - timeOut > 5){
    		CAN::errorCnt++;
    		CAN::errorAddress = CAN::sendAddress;	//タイムアウト処理｡雑
    		break;
    	}
    }
