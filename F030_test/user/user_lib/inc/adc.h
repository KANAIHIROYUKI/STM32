...
public:
	...
	int16_t read();
	void start(uint8_t ADC_SampleTime = ADC_SampleTime_13_5Cycles);
	uint16_t peek();
private:
	ADC_TypeDef* adc_adc;
	uint8_t adc_channel;
...
void ADC1Start(uint8_t ADC_Channel,uint8_t ADC_SampleTime = ADC_SampleTime_13_5Cycles);
void ADC2Start(uint8_t ADC_Channel,uint8_t ADC_SampleTime = ADC_SampleTime_13_5Cycles);
