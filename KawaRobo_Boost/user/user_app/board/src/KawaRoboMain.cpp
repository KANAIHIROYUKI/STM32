...
void KawaRobo::run(){
	...
	if(mode == MODE_TEST){
		...
		output = -getArmPosition() - armPitchGy.outputF()/3;		//ジャイロ補正あり
	}
		...
		if(battVoltage.peek() > battUnderVoltage){		//Arduinoからの受信がある､かつ電圧OK
			...
			if(getTogglePosition(TOGGLE_CONTROL_MODE) > 0.5){				//プロポのトグルスイッチの状態によって動作モードを変更する
				if(mode != MODE_RUN){
					...
					motorEnable();
					//serial->printf("SwC is UP\n\r");
					serial->printf("\n\rmode = RUN\n\r\n\r");
				}

			}else if(getTogglePosition(TOGGLE_CONTROL_MODE) > -0.1){
				if(mode != MODE_TEST){
