...
uint16_t rxFlag = 0,switchStat = 0,printMode = 0,canAddressJumper = 0;
...
void setup(){
	...
	canEnc[0].setup(enc[0],can1,0 + (canAddressJumper*4));
	canEnc[1].setup(enc[1],can1,1 + (canAddressJumper*4));
	canEnc[2].setup(enc[2],can1,2 + (canAddressJumper*4));
	canEnc[3].setup(enc[3],can1,3 + (canAddressJumper*4));

	canVlv.setup(can1,0 + canAddressJumper);
	canVlv.pinAdd(io[0]);
	canVlv.pinAdd(io[1]);
	canVlv.pinAdd(io[2]);
	canVlv.pinAdd(io[3]);
